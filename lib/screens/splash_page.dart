...
class _SplashPageState extends State<SplashPage> {
   ...
   void defaultsInitiation () async{
    ...
    var start = FirebaseFirestore.instance.collection('variables').snapshots().listen((QuerySnapshot querySnapshot) {
      querySnapshot.docs.forEach((users) async {
        setState(() {
          ...
          print("WAAAAAAALLLLLKKKKKK:${users["walkthroughs"]}");
    ...
    print("LOPOLOP: ${prefs.getBool(kIsOwner)}");
   if (ownerStatus == false) {
      // print("THIS RUN AND EMPLOYEE ID: ${prefs.getString(kEmployeeId)}");
      var employ = FirebaseFirestore.instance.collection('employees').where("id", isEqualTo: prefs.getString(kEmployeeId)??"").snapshots().listen((QuerySnapshot querySnapshot) {
        querySnapshot.docs.forEach((users) async {
          prefs.setString(kPermissions, users["permissions"]);
          print("permissions:${users["permissions"]}");
          Provider.of<BeauticianData>(context, listen: false).setAllEmployeePermission(CommonFunctions().convertPermissionsStringToJson(users["permissions"]));

          setState(() {
            if(users["permissions"]!= oldPermissions){
              ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text('Permissions were Updated')));
        ...
        child: Center(
          child: Column(
            ...
            children:[
              ...
              Text('Manage and Grow your Business', style: kHeadingTextStyleWhite.copyWith(color: kPureWhiteColor),)]
          ),
        ),
      ),
    );
