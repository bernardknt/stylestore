...
//     final TimeOfDay initialTime = TimeOfDay(hour: 8, minute: 0);
...
//     final TimeOfDay initialTime = TimeOfDay(hour: 18, minute: 0);
//     final TimeOfDay? pickedTime = await showTimePicker(
//       context: context,
//       initialTime: initialTime,
//     );
...
//     final ref = storage.ref().child('business_logos/$fileName');
...
//     }
//
//     return storesDb.doc(storeId).update({
...
//     }).then((value) => print("Service Added")).catchError((error) => print("Failed to add service: $error"));
//   }
//
...
//     final initials = prefs.getString(kBusinessNameConstant)?.split(' ').map((word) => word.isNotEmpty ? word[0].toUpperCase() : '').join('');
...
//                           ? imageUploaded
//                           ? Text("data")
//                       //Image.network(imageToUpload, height: 70, width: 70)
//                           : Opacity(
//                         opacity: 0.2,
...
//                         alignLeft: false,
//                       ),
//                       Text("|", style: TextStyle(fontSize: 25, color: kAppPinkColor)),
//                       SizedBox(width: 10),
//                       Expanded(
//                         child: TextFormField(
...
//                           child: Center(
//                             child: Text(
//                               "$selectedOpeningTime ${openingPeriod.name}",
//                               style: kNormalTextStyle.copyWith(
...
//                           child: Center(
//                             child: Text(
//                               "$selectedClosingTime ${closingPeriod.name}",
//                               style: kNormalTextStyle.copyWith(
//                                 color: kBlack,
//                                 fontWeight: FontWeight.bold,
//                               ),
...
//                     if (phoneNumber.isEmpty || addressController.text.isEmpty || businessNameController.text.isEmpty) {
...
class _OnboardingBusinessState extends State<OnboardingBusiness> {
  ...
  Future<void> _showTimePickerForOpeningTime(BuildContext context) async {
    // Set the initial time to today at 8 AM
    final TimeOfDay initialTime = TimeOfDay(hour: 8, minute: 0);
  ...
  Future<void> _showTimePickerForClosingTime(BuildContext context) async {
    // Set the initial time to today at 6 PM
    final TimeOfDay initialTime = TimeOfDay(hour: 18, minute: 0);
